  ...
  createAuthRouteHandler(async (req: AuthRequest, res) => {
    try {
      ...
      const webhooks = await webhookManager.getWebhooks({
        ...
        isActive: typeof isActive === 'string' ? isActive === 'true' : typeof isActive === 'boolean' ? isActive : undefined
      });
...
router.post(
  ...
  [
    body('name')
      .isString()
      .notEmpty()
      .withMessage('Webhook adı gereklidir')
      .trim(),
    body('url')
      .isURL()
      .withMessage('Geçerli bir URL girilmelidir'),
    body('events')
      .isArray({ min: 1 })
      .withMessage('En az bir event seçilmelidir'),
    body('events.*')
...
router.get(
  ...
  [
    param('id')
      .isMongoId()
      .withMessage('Geçersiz webhook ID')
  ],
  validateRequest,
  createAuthRouteHandler(async (req: AuthRequest, res) => {
    try {
...
router.patch(
  ...
  [
    ...
    body('name')
      ...
      .isString()
      .notEmpty()
      .withMessage('Webhook adı boş olamaz')
      .trim(),
    body('url')
      .optional()
      .isURL()
