...
const registerTextChannelEvents = (io: Server, dependencies: any) => {
  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('channel:history', async ({ channelId, limit = 50, before = null }: { channelId: string, limit?: number, before?: string | null }) => {
      try {
        // Kullanıcının bu kanala erişim yetkisi var mı kontrol et
        const channel = await performance.measureDatabaseQuery('Kanal bilgilerini getir', async () => {
        ...
        // Mesajları getir
        const messages = await performance.measureDatabaseQuery('Kanal mesajlarını getir', async () => {
        ...
        socket.emit('error', { message: 'Mesaj geçmişi alınırken bir hata oluştu', code: 'SERVER_ERROR' });
      }
    });

    // Yeni mesaj gönder
    socket.on('channel:message', async ({ channelId, content, mentions = [] }: { channelId: string, content: string, mentions?: string[] }) => {
      try {
        ...
        socket.emit('error', { message: 'Mesaj gönderilirken bir hata oluştu', code: 'SERVER_ERROR' });
      }
    });

    // Mesaj düzenle
    socket.on('channel:message:edit', async ({ messageId, content }: { messageId: string, content: string }) => {
      try {
        // Mesajı bul
        ...
        if (message.sender.toString() !== userId) {
          socket.emit('error', { message: 'Bu mesajı düzenleme yetkiniz yok', code: 'UNAUTHORIZED' });
          return;
        }
        ...
        socket.emit('error', { message: 'Mesaj düzenlenirken bir hata oluştu', code: 'SERVER_ERROR' });
      }
    });
    ...
    // Mesaj sabitle
    socket.on('channel:message:pin', async ({ messageId, isPinned }: { messageId: string, isPinned: boolean }) => {
      try {
        // Mesajı bul
        const message = await performance.measureDatabaseQuery('Mesaj getir', async () => {
          return await MessageHelper.findById(messageId).exec();
        ...
        socket.emit('error', { message: 'Mesaj sabitlenirken bir hata oluştu', code: 'SERVER_ERROR' });
      }
    });
