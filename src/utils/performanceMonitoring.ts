...
export function startMemoryMonitoring(interval = 60000): NodeJS.Timeout {
  return setInterval(() => {
    ...
    if (heapUsagePercentage > 85) {
      ...
      if (global.gc && Date.now() - metrics.lastGcTime > 300000) {
        // 5 dakikada bir
...
export function detectMemoryLeaks(interval = 60000, sampleCount = 5): NodeJS.Timeout {
  ...
  return setInterval(() => {
    ...
    if (samples.length >= sampleCount) {
      ...
      const isIncreasing =
        samples.length > 0 &&
        samples.every((heap, i) => {
          return (
            i === 0 ||
            (samples[i - 1] !== undefined && samples[i - 1] !== null && heap >= samples[i - 1])
          );
        });
      const growthRate =
        samples.length > 0 && samples.length > 1 && samples[0] && samples[samples.length - 1]
          ? ((samples[samples.length - 1] - samples[0]) / samples[0]) * 100
          : 0;
