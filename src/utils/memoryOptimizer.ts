  ...
  const timer = setInterval(() => {
    ...
    if (memoryUsage.heapUsed > thresholdBytes) {
      ...
      if (global.gc) {
        ...
        logger.info(
          'Çöp toplama işlemi zorlanamıyor. Node.js\'i --expose-gc parametresi ile başlatın'
        );
      }
    }
  }, interval);
  ...
  const timer = setInterval(() => {
    ...
    if (memoryUsageHistory.length >= 3) {
      const increasing = memoryUsageHistory.every((value, index, array) => {
        return (
          index === 0 ||
          (array[index - 1] !== undefined && array[index - 1] !== null && value > array[index - 1])
        );
      });
...
export function cleanupLargeObject(obj: any): void {
  ...
  // Nesneyi null olarak ayarla - güvenli bir şekilde
  // Object.setPrototypeOf yerine daha güvenli bir yaklaşım kullanıyoruz
  // Yeni bir nesne oluşturup referansı değiştiriyoruz
  const emptyObj = Object.create(null);
  Object.defineProperty(obj, '__proto__', {
    value: emptyObj.__proto__,
    writable: false,
    configurable: false,
  });
}
