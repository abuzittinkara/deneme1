...
import { Theme, ThemeType, DEFAULT_THEME, getTheme, generateCSSVariables } from '../config/themes';
import fs from 'fs';
...
class ThemeManager {
  ...
  generateThemeCSS(): boolean {
    try {
      ...
      logger.error('Tema CSS dosyası oluşturulurken hata oluştu', {
  ...
  serveThemeCSS(req: any, res: any): void {
    try {
      ...
      logger.error('Tema CSS dosyası servis edilirken hata oluştu', {
        error: (error as Error).message,
      });
