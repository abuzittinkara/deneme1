...
interface DMMessageAck {
  success: boolean;
  error?: string;
}

interface NewDMMessageData {
  friend: string;
  message: DMMessage;
}

export function initDMChat(socket: Socket, friendUsername: string): void {
  ...
  if (!dmContentArea) {
    ...
    if (selectedDMBar && selectedDMBar.parentNode) {
  ...
  function appendDMMessage(container: HTMLElement, msg: DMMessage): void {
  ...
  socket.emit('joinDM', { friend: friendUsername }, (res: DMResponse) => {
  ...
  function sendDM(): void {
    ...
    socket.emit('dmMessage', { friend: friendUsername, content: content }, (ack: DMMessageAck) => {
      if (ack && ack.success) {
  ...
  dmInput.addEventListener('keydown', (e: KeyboardEvent) => {
    if (e.key === 'Enter') {
      sendDM();
    }
  });
  ...
  socket.on('newDMMessage', (data: NewDMMessageData) => {
