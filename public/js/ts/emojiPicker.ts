...
interface DMChatInitializedEvent extends Event {
  detail?: any;
}
 ...
 */
function toggleEmojiPicker(inputElement: HTMLElement): void {
 ...
 * @param inputElement - Input element to insert emojis into
 */
function searchEmojis(query: string, emojiPicker: HTMLElement, inputElement: HTMLElement): void {
  const emojiContainer = emojiPicker.querySelector('.emoji-container');
  if (!emojiContainer) return;
...
function insertEmoji(emoji: string, emojiPicker: HTMLElement): void {
  ...
  const dmMessageInput = document.getElementById('dmMessageInput') as HTMLInputElement;
  
  if (textChannelInput && textChannelInput.offsetParent !== null) {
    inputElement = textChannelInput;
  } else if (dmMessageInput && dmMessageInput.offsetParent !== null) {
    inputElement = dmMessageInput;
  }
