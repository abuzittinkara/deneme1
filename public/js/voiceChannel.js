 ...
 */
export function initVoiceChannel(socket) {
 ...
 * @param {Object} socket - Socket.io socket
 */
function setupVoiceChannelEvents(socket) {
  ...
  socket.on('userLeftVoice', (data) => {
    ...
    // Consumer'ı kapat
    const consumer = consumers.get(data.userId);
    if (consumer) {
      consumer.close();
      consumers.delete(data.userId);
    }
  ...
  socket.on('userMuteChanged', (data) => {
  ...
  socket.on('userDeafenChanged', (data) => {
    const peer = voiceState.peers.get(data.userId);
    if (peer) {
  ...
  socket.on('newConsumer', async (data, callback) => {
    try {
      await consumeTrack(data, socket);
 ...
 * @param {Object} socket - Socket.io socket
 */
function setupVoiceChannelUIEvents(socket) {
 ...
 * @param {Object} routerRtpCapabilities - Router RTP yetenekleri
 */
 ...
 * @param {Object} transportOptions - Transport seçenekleri
 * @param {Object} socket - Socket.io socket
 */
async function createSendTransport(transportOptions, socket) {
  try {
    if (!mediasoupDevice) {
 ...
 * @param {Object} socket - Socket.io socket
 */
async function createRecvTransport(transportOptions, socket) {
  try {
    if (!mediasoupDevice) {
      throw new Error('Mediasoup cihazı yüklenmemiş');
    }
 ...
 * @param {Object} consumerOptions - Consumer seçenekleri
 * @param {Object} socket - Socket.io socket
 */
async function consumeTrack(consumerOptions, socket) {
  try {
    if (!recvTransport) {
    ...
    if (!mediasoupDevice.canConsume({
      producerId: consumerOptions.producerId,
      rtpCapabilities: mediasoupDevice.rtpCapabilities
    })) {
      throw new Error('Bu producer tüketilemez');
    }
    ...
    // Consumer'ı sakla
    consumers.set(consumerOptions.peerId, consumer);
 ...
 * @param {Object} socket - Socket.io socket
 */
export async function joinVoiceChannel(channelId, socket) {
  try {
 ...
 * @param {Object} socket - Socket.io socket
 */
export async function leaveVoiceChannel(socket) {
  try {
 ...
 * @param {Object} socket - Socket.io socket
 */
export function toggleMicrophone(socket) {
  try {
    if (!voiceState.isConnected) return;
